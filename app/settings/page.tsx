...
import { getStyleMemory, resetStyleMemory, type StyleMemory } from "@/lib/style-memory"
import {
...
function StyleMemoryCard() {
  const [memory, setMemory] = useState<StyleMemory>({ edits: [] })
  const [hydrated, setHydrated] = useState(false)

  useEffect(() => {
    queueMicrotask(() => {
      setMemory(getStyleMemory())
      setHydrated(true)
    })
  }, [])

  function handleReset() {
    if (!window.confirm("Clear writing style memory? Future First Impression analyses will revert to the baseline voice until you make new edits.")) return
    resetStyleMemory()
    setMemory({ edits: [] })
    toast.success("Writing style memory cleared")
  }

  const editCount = memory.edits.length
  const profile = memory.profile
  const generatedAt = memory.profileGeneratedAt
    ? new Date(memory.profileGeneratedAt).toLocaleString()
    : null

  return (
    <Card>
      <CardHeader>
        <CardTitle>Writing style memory</CardTitle>
        <CardDescription>
          Each time you edit a First Impression and click OK, the change is saved. After every few
          edits, the AI distills your voice into a profile and uses it on future First Impression
          analyses so the output sounds like you.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-3">
        {!hydrated ? (
          ...
          <p className="text-xs text-muted-foreground">
            No edits captured yet. Edit a First Impression on any site to start training your style.
          </p>
        ) : (
          <div className="space-y-2 text-xs">
            <div className="text-muted-foreground">
              <strong className="text-foreground">{editCount}</strong> edit{editCount === 1 ? "" : "s"} captured
              {generatedAt && (
                <>
                  {" · "}profile generated <span className="text-foreground">{generatedAt}</span>
                </>
              )}
            </div>
            {profile ? (
                ...
                {profile.rules?.length > 0 && (
                  <div>
                    ...
                    <ul className="ml-4 list-disc">
                      {profile.rules.map((r, i) => <li key={i}>{r}</li>)}
                    </ul>
                  </div>
                )}
                {(profile.vocab?.prefer?.length || profile.vocab?.avoid?.length) ? (
                  <p>
                    {profile.vocab.prefer?.length > 0 && (
                      <><strong>Prefer:</strong> {profile.vocab.prefer.join(", ")}</>
                    )}
                    {profile.vocab.prefer?.length > 0 && profile.vocab.avoid?.length > 0 && " · "}
                    {profile.vocab.avoid?.length > 0 && (
                      <><strong>Avoid:</strong> {profile.vocab.avoid.join(", ")}</>
                    )}
                  </p>
                ) : null}
              </div>
              ...
              <p className="text-muted-foreground">
                Profile will be generated after a few more edits.
              </p>
            )}
          </div>
        )}
        <Button variant="outline" size="sm" onClick={handleReset} disabled={!hydrated || editCount === 0}>
          Reset writing style memory
        </Button>
      </CardContent>
    </Card>
  )
