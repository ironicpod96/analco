...
import { imageGroupToSvg, loadImage, renderImageWithFit, type ImageOverride } from "@/lib/presentation-export"
...
export async function exportSlidesToNewPresentation(
  ...
  validateConfig(config)
  const token = await requestGoogleToken(config.clientId)
  const presentation = await createPresentation(title, token)
  for (const slide of slides) {
    await appendSlideToPresentation(presentation.presentationId, slide, token, imageOverrides)
  }
  return presentation.presentationId
}

export async function exportSlidesToPickedPresentation(
  slides: PresentationSlide[],
  config: GoogleSlidesConfig,
  imageOverrides?: Map<string, ImageOverride>
): Promise<string> {
  ...
  const presentationId = await pickPresentation(config.apiKey, token)
  for (const slide of slides) {
    await appendSlideToPresentation(presentationId, slide, token, imageOverrides)
  }
  return presentationId
}
...
async function appendSlideToPresentation(
  ...
  const { slide: resolvedSlide, uploadedFileIds } = await resolveSlideImages(slide, token, imageOverrides)
  setTimeout(() => uploadedFileIds.forEach((id) => deleteDriveFile(id, token)), 90_000)

  const deck = await getDeckMeta(presentationId, token)
  const scale = scaleFor(resolvedSlide, deck)
  const exportId = Date.now().toString(36)

  const withImages = () => buildSlideRequests(resolvedSlide, scale, exportId, true)
  let response = await sendBatchUpdate(presentationId, withImages(), token)
  if (response.ok) return

  let message = await googleError(response, "Could not export to Google Slides")
  const isImageFetchError = /provided image was not found|createImage/i.test(message)
  if (!isImageFetchError) throw new Error(message)

  for (const waitMs of [1500, 3500]) {
    await new Promise((resolve) => setTimeout(resolve, waitMs))
    response = await sendBatchUpdate(presentationId, withImages(), token)
    if (response.ok) return
    message = await googleError(response, "Could not export to Google Slides")
  ...
  const fallbackResponse = await sendBatchUpdate(
    presentationId,
    buildSlideRequests(resolvedSlide, scale, `${exportId}-noimg`, false),
    token
  )
...
async function resolveSlideImages(
  ...
  token: string,
  imageOverrides?: Map<string, ImageOverride>
): Promise<{ slide: PresentationSlide; uploadedFileIds: string[] }> {
  const uploadedFileIds: string[] = []
  const cache = new Map<string, string>() // cacheKey → Drive URL

  const elements = await Promise.all(
    slide.elements.map(async (element) => {
      if (element.type === "imageGroup") {
        try {
          const svg = imageGroupToSvg(element, slide.fontFamily)
          const blob = await svgToPngBlob(svg, element.width, element.height)
          const { fileId, url } = await uploadBlobToDrive(blob, "image/png", `slide-group-${Date.now()}.png`, token)
      ...
      const override = imageOverrides?.get(element.id)
      const effectiveUrl = override?.url ?? element.url ?? ""
      const effectiveFit = override?.objectFit ?? (element.objectFit as "contain" | "cover" | "fill" | undefined) ?? "contain"

      if (!effectiveUrl) return element

      const cacheKey = `${effectiveUrl.slice(0, 64)}:${effectiveFit}:${element.width}x${element.height}`
      if (cache.has(cacheKey)) return { ...element, url: cache.get(cacheKey)! }

      try {
        const SCALE = 2
        const blob = await renderImageWithFit(
          effectiveUrl,
          Math.round(element.width * SCALE),
          Math.round(element.height * SCALE),
          effectiveFit
        )
        const { fileId, url } = await uploadBlobToDrive(blob, "image/jpeg", `slide-img-${Date.now()}.jpg`, token)
        uploadedFileIds.push(fileId)
  ...
  return { slide: { ...slide, elements }, uploadedFileIds }
}
...
async function uploadBlobToDrive(
  ...
  return { fileId, url: `https://lh3.googleusercontent.com/d/${fileId}=w2400` }
}
...
function elementRequests(
  ...
  if (element.type === "image") {
    if (!includeImages || !element.url || element.url.startsWith("data:")) return []
    return imageRequests(element, pageObjectId, exportId, scale)
  }
  ...
  if (element.type === "imageGroup") {
    // Fallback if upload in resolveSlideImages failed — render sub-elements individually
    return element.elements.flatMap((sub) => elementRequests(sub, pageObjectId, slide, exportId, scale, includeImages))
  }
        ...
        shapeProperties: {
          shapeBackgroundFill:
            !element.fill || element.fill === "transparent"
              ...
              : { solidFill: { color: rgb(element.fill) } },
          outline:
            element.stroke && element.stroke !== "transparent"
              ? {
                  outlineFill: { solidFill: { color: rgb(element.stroke) } },
                  weight: { magnitude: (element.strokeWidth ?? 1) * scale.scale, unit: "PT" },
                }
              : { propertyState: "NOT_RENDERED" },
        },
...
function textRequests(
  ...
  const id = objectId(`${exportId}-${element.id}`)
  const box = textBox(element, scale)
  const textContent = element.richText ? plainTextFromRichText(element.richText) : element.text
...
function textBox(element: Extract<PresentationElement, { type: "text" }>, scale: ScaleContext) {
  // Slides applies a default 0.1" (7.2pt of real Slides PT) text inset on each
  // side. Grow the shape outward by that amount so the inner text area matches
  // the webapp's box. The expansion must be expressed in webapp units (it will
  // be scaled by size()/transform() afterwards), so divide by scale.scale.
  const inset = -7.2 / scale.scale
  return {
    x: element.x + inset,
    y: element.y + inset,
...
function objectId(value: string): string {
  ...
  for (let i = 0; i < sanitized.length; i++) {
    hash = ((hash << 5) + hash + sanitized.charCodeAt(i)) >>> 0
  }
  const suffix = `_${hash.toString(36)}`
  return sanitized.slice(0, 48 - suffix.length) + suffix
}
