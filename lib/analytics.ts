...
const GENERIC_SLDS = new Set(["com", "net", "org", "gov", "edu", "co", "ac"])

// The previous siteNameFromUrl returned the second-to-last hostname part regardless of SLD,
// so e.g. dbs.com.sg → "Com". Insights generated with that label may still be cached in
// localStorage; expose the legacy form so storage can detect & clean those up on rename.
export function legacySiteNameFromUrl(url: string): string {
...
export function buildSiteMeta(audits: SiteAudit[]): SiteMeta[] {
  ...
  return audits.map((audit) => {
    ...
    return {
      ...
      label: audit.customLabel ?? siteNameFromUrl(audit.url),
    }
  })
}
...
export function applyCrossSiteInsightOverrides(
  ...
  return insights
    .map((insight, index): CrossSiteInsight | null => {
      const override = overrides[`${category}:${index}`]
      if (override?.removed) return null
      if (!override) return { ...insight, originalIndex: index }
    ...
    .filter((x): x is CrossSiteInsight => x !== null)
}
