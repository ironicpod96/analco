...
const TASK_COMPLETION_RULES: Array<InsightRule<TaskCompletionSignals>> = [
  ...
  {
    when: (s) => s.ease === 2 && s.duration === 0,
    headline: "keep the path clear but not direct enough",
    text: "Each step is easy but the path is too long. Per Progressive Disclosure, when too many steps sit between a user and their goal, shortening the path matters even when each step is obvious.",
    principleTitle: "Progressive Disclosure",
  },
  {
    when: (s) => s.ease === 0 && s.duration === 2,
    headline: "demand more mental effort than the task itself requires",
    text: "The path is short but each step is hard. Per Cognitive Load, when users think about the interface instead of their task, speed and accuracy both drop.",
  ...
  {
    when: (s) => s.ease === 1 || s.duration === 1,
    headline: "leave the task passable but not smooth",
    text: "The task is passable but not smooth. Per Error Prevention, each rough patch is a point where the design failed to anticipate what the user needed next.",
